...
[Route("api")]
public class DataController(LanePetsDbContext db, SessionService sessions) : ApiControllerBase
{
    private bool Admin(string token) { sessions.RequireAdmin(token); return true; }
    [HttpGet("dashboard")]
    ...
    [HttpGet("pets")] public Task<IActionResult> Pets(string token="",string? busca=null,int limite=500,int offset=0)=>List(token, busca, limite, offset, async()=>await db.Pets.AsNoTracking().ToListAsync(), p=>new {id=p.Id,dono=p.Dono,pet=p.PetNome,tipo=p.Tipo,raca=p.Raca,telefone=p.Telefone,endereco=p.Endereco,pacote_json=p.PacoteJson,unidade=p.Unidade,cliente_id=p.ClienteId});
    [HttpGet("clientes")] public Task<IActionResult> Clientes(string token="",string? busca=null,int limite=500,int offset=0)=>List(token,busca,limite,offset,async()=>await db.Clientes.AsNoTracking().ToListAsync(),c=>new{id=c.Id,nome=c.Nome,telefone=c.Telefone,endereco=c.Endereco,observacoes=c.Observacoes,origem=c.Origem,status=c.Status});
    [HttpGet("servicos")] public Task<IActionResult> Servicos(string token="",string? busca=null,int limite=500,int offset=0)=>List(token,busca,limite,offset,async()=>await db.Servicos.AsNoTracking().ToListAsync(),s=>new{id=s.Id,nome=s.Nome,preco=s.Preco,porte=s.Porte,adicionais_json=s.AdicionaisJson,pacote=s.Pacote,adicional=s.Adicional});
    [HttpGet("pedidos")]
    public async Task<IActionResult> Pedidos(string token="") { try { Admin(token); return OkApi(await db.Pedidos.AsNoTracking().OrderByDescending(p => p.CriadoEm).ToListAsync()); } catch(Exception ex) { return ErrorApi(ex); } }
    [HttpGet("unidades")] public IActionResult Unidades(string token=""){try{Admin(token);return OkApi(new{unidades=new[]{"Franco","Caieiras"}});}catch(Exception ex){return ErrorApi(ex);}}

    private async Task<IActionResult> List<T>(string token,string? busca,int limite,int offset,Func<Task<List<T>>> load,Func<T,object> map){try{Admin(token);var rows=await load();var b=NormText(busca);var items=rows.Where(x=>string.IsNullOrEmpty(b)||JsonSerializer.Serialize(x).ToLowerInvariant().Contains(b)).ToList();var total=items.Count;limite=Math.Clamp(limite,1,2000);offset=Math.Max(0,offset);return OkApi(new{ok=true,total,offset,limite,items=items.Skip(offset).Take(limite).Select(map)});}catch(Exception ex){return ErrorApi(ex);}}
    ...
    private static string NormText(string? s)=>string.IsNullOrWhiteSpace(s)?"":s.Normalize(System.Text.NormalizationForm.FormD).Where(c=>System.Globalization.CharUnicodeInfo.GetUnicodeCategory(c)!=System.Globalization.UnicodeCategory.NonSpacingMark).Aggregate("",(a,c)=>a+c).ToLowerInvariant().Trim();
    private static string NormUnit(string? s)=>NormText(s) switch {"franco"=>"Franco","caieiras"=>"Caieiras",_=>""};
    private static string NormStatus(string? s)=>NormText(s) switch {"pendente" or "aguardando" or "agendado"=>"Pendente","em andamento" or "em_andamento" or "andamento" or "iniciado" or "em processo"=>"Em andamento","entregue" or "finalizado" or "finalizado/entregue" or "concluido" or "concluído"=>"Entregue","cancelado"=>"Cancelado",_=>""};
    private static string DateOnly(string? s){if(DateTime.TryParse(s,out var d))return d.ToString("yyyy-MM-dd");return (s??"").Length>=10?s![..10]:"";}
}
